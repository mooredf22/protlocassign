...
## Obtaining constrained proportional assignments (CPA) of proteins to compartments  
...
Note that the protein "AIF1" has all missing values, which is why an error is reported for that one protein.
...
The values in `refLocationProfilesAcup` represent, in principle, the relative amount of each cellular compartment distributed to each centrifugation fraction.
...
```
 ...
 We also must create a vector `totProt` with five values, corresponding to
  N, M, L (=L1+L2), P, S. Here are the original values for total protein in each of the nine channels:
...
par(mfrow=c(1,1))
errorMatAll <- mixtureHeatMap(Acup=refLocationProfilesAcup_5frac, 
