...
First install the Bioconductor untility `BiocManager`:
...
Once you have installed these packages, you do not need to re-install them the next time you start up R. However, you will need to load them using `library` before use.
...
Note that the protein "AIF1" has all missing values, which is why the spg function returns an error for that one protein.
