...
To obtain the `protlocassign` package, first installing the `devtools` package from CRAN by typing:
...
Other packages may downloaded by using the Bioconductor repository. The first step is to install the "BiocManager" package from CRAN, by typing:
...
Consider for example a test data set drawn from the TMT MS2 data from Tannous et al. (2020) experiment AT5. To get started, load the package and attach the embedded test protein profile data set; we see that it has 40 rows and 11 columns: 
...
If one desires to work with the full set of 7893 protein profiles in the Tannous et al. data set, we may download it from the MassIVE repository. To do so, execute the following code, first removing the comment character "#":

```
# urlQuantPSMprot <-  "https://massive.ucsd.edu/ProteoSAFe/DownloadResultFile?file=f.MSV000083842/updates/2022-03-14_ablatannous_c39637a3/quant/protNSA_AT5tmtMS2.csv&forceDownload=true"

# protNSA_AT5tmtMS2 <- read.csv(urlQuantPSMprot)
...
```
devtools::install("protlocassign")
...
Finally, be sure to install `BiocParallel` for parallel computing as described in Tutorial 1.
