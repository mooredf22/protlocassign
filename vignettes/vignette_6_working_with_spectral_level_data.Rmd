...
```
As an example, here are commands to read in the Tannous AT5 TMT-MS2 data from an external file stored on
...
```
The data frame `spectraNSA_AT5tmtMS2` contains the full data set. Since we shall use it in the remainder of this vignette, we simplify the name as follows:
...
The `lmer` random effects function within `profileSummarize` will generate error messages for some proteins due to numerical singularity issues. In these cases, `profileSummarize` automatically re-calculates the profile average using standard (non-nested) averaging.

```{r, echo=TRUE, message=FALSE, warning=FALSE}
protNSA_1 <- 
          profileSummarize(protsCombineCnew=flagSpectraPeps,
          numRefCols=7, numDataCols=9, eps=eps, GroupBy="protId",
          outlierExclude="spectraAndPeptide", cpus=4)
